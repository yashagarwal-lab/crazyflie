...
Marker IDs are identified at startup via interactive scan — no hardcoding needed.
...
# Marker IDs are assigned at runtime via scan — not hardcoded
DRONE1_MARKER   = None
DRONE2_MARKER   = None

DRONE1_TARGET_Z =  0.5
DRONE2_TARGET_Z =  1.90
...
# ── Scan state — used only during startup marker identification ───────────────
class ScanState:
    def __init__(self):
        self.lock    = threading.Lock()
...
# ── Scan callback — collects all visible markers ──────────────────────────────
...
# ── Flight callback — routes positions to correct drone pose ──────────────────
...
# ── Interactive marker scan and assignment ────────────────────────────────────
def scan_and_assign_markers(client):
    """
    Runs a 5-second scan, prints all visible markers with remapped coordinates,
    then asks user to type which marker ID belongs to Drone1 and Drone2.
    Returns (drone1_marker_id, drone2_marker_id) or raises SystemExit on failure.
    """
    global DRONE1_MARKER, DRONE2_MARKER
    ...
    while True:
        try:
            d1_input = input("  Enter marker ID for Drone1: ").strip()
            d1_id = int(d1_input)
            if d1_id not in found:
                print(f"  [ERR] {d1_id} not in scan results. Try again.")
                continue
            break
        except ValueError:
    ...
    while True:
        try:
            d2_input = input("  Enter marker ID for Drone2: ").strip()
            d2_id = int(d2_input)
            if d2_id not in found:
                print(f"  [ERR] {d2_id} not in scan results. Try again.")
                continue
            if d2_id == d1_id:
                print("  [ERR] Drone2 marker must be different from Drone1.")
                continue
            break
        except ValueError:
            print("  [ERR] Please enter a valid integer marker ID.")
    ...
    print(f"\n  [OK] Drone1 → marker {DRONE1_MARKER}  "
          f"x={d1_pos[0]:+.3f}  y={d1_pos[1]:+.3f}  z={d1_pos[2]:+.3f}")
    print(f"  [OK] Drone2 → marker {DRONE2_MARKER}  "
          f"x={d2_pos[0]:+.3f}  y={d2_pos[1]:+.3f}  z={d2_pos[2]:+.3f}")

    # Switch to flight callback now that IDs are known
    marker_to_pose[DRONE1_MARKER] = pose1
    marker_to_pose[DRONE2_MARKER] = pose2
    client.labeledMarkerListener = labeled_marker_callback

    # Seed pose objects with scan data so they are immediately valid
    with pose1.lock:
        pose1.x, pose1.y, pose1.z = d1_pos
        pose1.valid = True
    with pose2.lock:
        pose2.x, pose2.y, pose2.z = d2_pos
        pose2.valid = True
...
def plan_path(moving_drone_name,
    ...
    avoid_z = min(obs_z + AVOID_OFFSET, MAX_ALTITUDE - 0.1)
    ...
    while True:
        ...
        if len(parts) == 3:
            ...
            waypoints, avoided = plan_path(
                moving_name, cx, cy, cz, tx, ty, tz, ox, oy, oz)
            ...
            if drone_id == "1":
                with nav1.lock:
                    nav1.waypoint_queue = waypoints[1:]
...
def main():
    ...
    # Step 1 — scan and assign marker IDs interactively
    try:
        scan_and_assign_markers(client)
    except SystemExit:
        client.stop()
        return

    # Step 2 — sanity check using scanned positions
    if not sanity_check():
        client.stop()
        return

    # Step 3 — save home positions
    with pose1.lock:
