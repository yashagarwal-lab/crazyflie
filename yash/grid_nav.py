#!/usr/bin/env python3
"""
2-drone MoCap-only grid-based waypoint navigation using OptiTrack + cflib.
...
Grid layout (5x5, origin at 13, spacing 0.5m):
   1  2  3  4  5      y=+1.0
   6  7  8  9 10      y=+0.5
  11 12 13 14 15      y= 0.0
  16 17 18 19 20      y=-0.5
  21 22 23 24 25      y=-1.0
  x= -1.0 -0.5 0.0 +0.5 +1.0

Safety features:
  - Pre-flight position sanity check
...
# ── Grid map — 25 positions, origin at 13, spacing 0.5m ──────────────────────
# Drones always face +X axis
GRID = {
...
def land(scf, pose, name):
    ...
    cf.commander.send_stop_setpoint()
    time.sleep(0.3)
    # Reset firmware back to default state so no power cycle needed
    try:
...
def input_thread(home1_x, home1_y, home2_x, home2_y, status_lock):
    ...
    while True:
        ...
        if raw.lower() == "status":
            ...
            # Find closest grid position for each drone
            def nearest_grid(x, y):
                closest = min(GRID.items(), key=lambda g: (g[1][0]-x)**2 + (g[1][1]-y)**2)
                return closest[0]

            print(f"\n  [Drone1] pos=({p1x:+.3f},{p1y:+.3f},{p1z:+.3f})  "
                  f"tgt=({t1x:+.3f},{t1y:+.3f},{t1z:+.3f})  "
                  f"nearest grid={nearest_grid(p1x,p1y)}")
            print(f"  [Drone2] pos=({p2x:+.3f},{p2y:+.3f},{p2z:+.3f})  "
                  f"tgt=({t2x:+.3f},{t2y:+.3f},{t2z:+.3f})  "
                  f"nearest grid={nearest_grid(p2x,p2y)}")
        ...
        if len(parts) == 3:
            ...
            # Collision proximity warning vs other drone
            if drone_id == "1":
                with pose2.lock:
            ...
            if sep < COLLISION_RADIUS:
                print(f"  [WARN] Grid {grid_num} is only {sep:.3f}m from the other drone!")
            ...
            if drone_id == "1":
                with nav1.lock:
                    ...
                    nav1.target_y = ty
                    nav1.target_z = tz
                print(f"  [NAV]  Drone1 → Grid {grid_num:>2} ({tx:+.3f},{ty:+.3f}) at z={tz}m")
            else:
                with nav2.lock:
                    nav2.target_x = tx
                    nav2.target_y = ty
                    nav2.target_z = tz
                print(f"  [NAV]  Drone2 → Grid {grid_num:>2} ({tx:+.3f},{ty:+.3f}) at z={tz}m")
            continue
...
def drone_flight_loop(scf, pose, nav, name, stop_event, status_lock):
    ...
    # Reset EKF with live MoCap data already flowing
    cf.param.set_value('kalman.resetEstimation', '1')
    time.sleep(0.1)
    cf.param.set_value('kalman.resetEstimation', '0')
    ...
    try:
        while not stop_event.is_set() and not kill_event.is_set():
            ...
            # Find current nearest grid position
            nearest = min(GRID.items(), key=lambda g: (g[1][0]-cx)**2 + (g[1][1]-cy)**2)
...
def main():
    ...
    # Print grid at startup for reference
    print_grid()
    ...
    # Save actual home positions from live MoCap
    with pose1.lock:
        home1_x, home1_y = pose1.x, pose1.y
    with pose2.lock:
        home2_x, home2_y = pose2.x, pose2.y

    # Set nav targets to actual ground XY
    with nav1.lock:
        nav1.target_x = home1_x
        nav1.target_y = home1_y
    with nav2.lock:
    ...
    try:
        ...
        # Cleanup on crash — reset firmware so no power cycle needed
        try:
            with SyncCrazyflie(DRONE1_URI, cf=Crazyflie(rw_cache='./cache1')) as scf1:
