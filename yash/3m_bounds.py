...
# ── Safety boundary — drone gracefully lands if it exceeds any of these ───────
BOUNDARY_XY = 1.8   # metres — applies to both x and y (positive and negative)
BOUNDARY_Z  = 1.8   # metres — max allowed altitude before graceful land
    ...
    def flight_loop(self, scf, all_drones, status_lock):
        ...
        try:
            while not self.stop_event.is_set() and not kill_event.is_set():
                ...
                # ── Boundary check — graceful land this drone only ────────────
                out_of_bounds = (
                    abs(cx) >= BOUNDARY_XY or
                    abs(cy) >= BOUNDARY_XY or
                    cz      >= BOUNDARY_Z
                )
                if out_of_bounds:
    ...
    while True:
        ...
        if len(parts) == 3:
            ...
            if tz > MAX_ALTITUDE:
                ...
                continue
            if tz < 0.1:
                print("  [ERR]  Height too low — minimum 0.1m")
                continue
